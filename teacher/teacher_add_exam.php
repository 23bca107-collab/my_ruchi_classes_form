...
// Log page access
logTeacherActivity('VIEW_EXAMS', 'Viewed exams management page');
...
if (isset($_GET['delete_id'])) {
    ...
    if ($stmt->execute()) {
        logTeacherActivity('DELETE_EXAM', "Deleted exam ID: $id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_exam'])) {
        ...
        if ($stmt->execute()) {
            logTeacherActivity('UPDATE_EXAM', "Updated exam ID: $id");
        ...
        // ADD NEW
        $stmt = $conn->prepare("INSERT INTO exams (class, medium, subject, topic, exam_date, exam_time, total_marks) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssi", $class, $medium, $subject, $topic, $exam_date, $exam_time, $total_marks);
        if ($stmt->execute()) {
            $new_id = $stmt->insert_id;
            logTeacherActivity('ADD_EXAM', "Added new exam ID: $new_id with total marks: $total_marks");
            $_SESSION['success_message'] = "Exam added successfully! Total Marks: $total_marks";
        } else {
            $_SESSION['error_message'] = "Failed to add exam!";
        }
        $stmt->close();
    }
...
<head>
    ...
    <title>Exams Management - Ruchi Classes</title>
    ...
    <style>
        ...
        * {
        ...
        .exam-card-value .action-buttons {
    ...
    <div class="main-content" id="mainContent">
        ...
        <div class="container">
            ...
            <div class="page-title">
                ...
                <p>Add, edit, and manage all exams with total marks</p>
            </div>
            
            <!-- Success/Error Messages -->
            <?php if (isset($_SESSION['success_message'])): ?>
            <div class="message-popup success" id="successMessage">
                <i class="fas fa-check-circle"></i> <?php echo $_SESSION['success_message']; unset($_SESSION['success_message']); ?>
            </div>
            <?php endif; ?>

            <?php if (isset($_SESSION['error_message'])): ?>
            <div class="message-popup error" id="errorMessage">
                ...
                <form method="POST" id="examForm">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="subject" id="subject" class="form-control" value="<?= $edit_exam ? htmlspecialchars($edit_exam['subject']) : '' ?>" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="topic"><i class="fas fa-tag"></i> Topic</label>
                            <input type="text" name="topic" id="topic" class="form-control" value="<?= $edit_exam ? htmlspecialchars($edit_exam['topic']) : '' ?>" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="exam_date"><i class="fas fa-calendar-alt"></i> Exam Date</label>
                            <input type="date" name="exam_date" id="exam_date" class="form-control" value="<?= $edit_exam ? $edit_exam['exam_date'] : '' ?>" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="exam_time"><i class="fas fa-clock"></i> Exam Time</label>
                            <input type="time" name="exam_time" id="exam_time" class="form-control" value="<?= $edit_exam ? $edit_exam['exam_time'] : '' ?>" required>
                        </div>

                        <!-- Total Marks Field -->
                        <div class="form-group">
                            <label for="total_marks"><i class="fas fa-star"></i> Total Marks</label>
                            <input type="number" name="total_marks" id="total_marks" class="form-control" 
                                   value="<?= $edit_exam ? htmlspecialchars($edit_exam['total_marks']) : '100' ?>" 
                                   min="1" max="500" placeholder="Enter total marks" required>
                            <small>
                                <i class="fas fa-info-circle"></i> Maximum marks for this exam (1-500)
                            </small>
                        </div>
                    </div>
                    
                    <div style="display: flex; gap: var(--space-4); margin-top: var(--space-4); flex-wrap: wrap;">
                        <button type="submit" class="btn btn-primary" name="<?= $edit_exam ? 'update_exam' : 'add_exam' ?>" style="flex: 1; min-width: 200px;">
                            <i class="fas <?= $edit_exam ? 'fa-sync' : 'fa-plus' ?>"></i> <?= $edit_exam ? 'Update Exam' : 'Add Exam' ?>
                        </button>
                        
                        <?php if ($edit_exam): ?>
                        <a href="all_exam.php" class="btn btn-danger" style="flex: 1; min-width: 200px;">
            ...
            <div class="card">
                                ...
                                <tr class="fade-in">
                                    ...
                                    <td>
                                        <span class="total-marks-badge">
                                            <i class="fas fa-star"></i> <?= $row['total_marks'] ?>
                                        </span>
                                    </td>
                                    <td>
                ...
                <!-- Mobile Cards View -->
                <div class="mobile-exams-container">
                    <?php 
                    if ($result && $result->num_rows > 0) {
                    ...
                    <div class="exam-card fade-in">
                        <div class="exam-card-row">
                            <div class="exam-card-label">ID</div>
                            <div class="exam-card-value">#<?= $row['id'] ?></div>
                        </div>
                        
                        <div class="exam-card-row">
                            <div class="exam-card-label">Class</div>
                            <div class="exam-card-value">Class <?= $row['class'] ?></div>
                        </div>
                        
                        <div class="exam-card-row">
                            <div class="exam-card-label">Medium</div>
                            <div class="exam-card-value"><?= $row['medium'] ?></div>
                        </div>
                        
                        <div class="exam-card-row">
                            <div class="exam-card-label">Subject</div>
                            <div class="exam-card-value"><?= htmlspecialchars($row['subject']) ?></div>
                        </div>
                        
                        <div class="exam-card-row">
                            <div class="exam-card-label">Topic</div>
                            <div class="exam-card-value"><?= htmlspecialchars($row['topic']) ?></div>
                        </div>
                        
                        <div class="exam-card-row">
                            <div class="exam-card-label">Exam Date</div>
                            <div class="exam-card-value"><?= date('M j, Y', strtotime($row['exam_date'])) ?></div>
                        </div>
                        
                        <div class="exam-card-row">
                            <div class="exam-card-label">Exam Time</div>
                            <div class="exam-card-value"><?= date('h:i A', strtotime($row['exam_time'])) ?></div>
                        </div>

                        <!-- Total Marks in mobile card -->
                        <div class="exam-card-row">
                            <div class="exam-card-label">Total Marks</div>
                            <div class="exam-card-value">
                                <span class="total-marks-badge">
                                    <i class="fas fa-star"></i> <?= $row['total_marks'] ?>
                                </span>
                            </div>
                        ...
                        <div class="exam-card-row">
                            <div class="exam-card-label">Actions</div>
                            <div class="exam-card-value">
                                <div class="action-buttons">
                                    <a href="all_exam.php?edit_id=<?= $row['id'] ?>" class="btn btn-edit btn-sm">
                                        <i class="fas fa-edit"></i> Edit
                    ...
                    <div class="exam-card" style="text-align: center; padding: 30px;">
                        <i class="fas fa-info-circle" style="font-size: 3rem; color: var(--border); margin-bottom: 15px;"></i>
                        <h3 style="color: var(--text-secondary);">No exams found</h3>
                        <p style="color: var(--text-muted); margin-top: 10px;">Add your first exam above.</p>
                    </div>
                    <?php } ?>
                </div>
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (examForm) {
                examForm.addEventListener('submit', function(e) {
                    ...
                    // Special validation for total marks
                    ...
                    if (totalMarks) {
                        ...
                        if (isNaN(marks) || marks < 1 || marks > 500) {
                            totalMarks.style.borderColor = '#ef4444';
                            Swal.fire({
                                icon: 'error',
                                title: 'Invalid Total Marks',
                                text: 'Total marks must be between 1 and 500',
