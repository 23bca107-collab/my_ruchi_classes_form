...
// Log dashboard access (with error handling)
try {
    logAdminActivity('DASHBOARD_ACCESS', 'Accessed admin dashboard');
} catch (Exception $e) {
    // Silently handle logging errors
}

// Check admin permissions for dashboard actions
...
// 1. Handle adding new teacher (Simplified - No tokens)
    ...
    if (!validateAdminCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (!$normalized['valid']) {
            ...
            if ($existingTeacher) {
                ...
                $stmt2 = $conn->prepare("INSERT INTO teachers (email, password, created_at, status) VALUES (?, NULL, NOW(), 'pending')");
                $stmt2->bind_param("s", $email);
                
                if ($stmt2->execute()) {
                    $success = "Teacher added successfully! They can now login with their email and will set up their password on first login.";
                    if ($normalized['auto_corrected']) {
                        $success .= ' ' . $normalized['correction_message'];
                    }
                    logAdminActivity('TEACHER_ADDED', "New teacher added: $email");
                } else {
                    $error = "Error adding teacher: " . $conn->error;
                    logAdminActivity('TEACHER_ADD_ERROR', "Failed to add teacher: $email");
                }
            }
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_teacher'])) {
    ...
    if (!validateAdminCSRFToken($_POST['csrf_token'] ?? '')) {
        $error = "Security token invalid or expired. Please refresh the page.";
        logAdminActivity('CSRF_FAILURE', 'Failed CSRF validation on teacher delete');
    } elseif (!$can_delete_teachers) {
        $error = "Only Super Admin can delete teachers!";
        logAdminActivity('PERMISSION_DENIED', 'Attempted to delete teacher without permission');
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_status'])) {
    ...
    if (!validateAdminCSRFToken($_POST['csrf_token'] ?? '')) {
        $error = "Security token invalid or expired. Please refresh the page.";
        logAdminActivity('CSRF_FAILURE', 'Failed CSRF validation on status toggle');
    } elseif (!$can_manage_teachers) {
        $error = "You don't have permission to modify teacher status!";
        logAdminActivity('PERMISSION_DENIED', 'Attempted to toggle teacher status without permission');
    } else {
        $teacher_id = intval($_POST['teacher_id'] ?? 0);
        ...
        if (!in_array($new_status, ['active', 'inactive', 'pending'])) {
            ...
            if ($update_stmt->execute()) {
                $status_text = $new_status === 'active' ? 'activated' : ($new_status === 'inactive' ? 'deactivated' : 'set to pending');
                $success = "Teacher $status_text successfully!";
                logAdminActivity('TEACHER_STATUS_CHANGED', "Teacher ID $teacher_id status changed to $new_status");
            } else {
                $error = "Error updating teacher status!";
                logAdminActivity('TEACHER_STATUS_ERROR', "Failed to update teacher ID $teacher_id status");
            }
        }
    }
}
...
$teachers_result = $conn->query("SELECT id, first_name, last_name, email, profile_completed, status, created_at, password, setup_token, token_expiry FROM teachers ORDER BY id DESC LIMIT $limit OFFSET $offset");
$teachers = $teachers_result ? $teachers_result->fetch_all(MYSQLI_ASSOC) : [];

// Get total count for pagination
$total_result = $conn->query("SELECT COUNT(*) as count FROM teachers");
...
// Teacher stats
$total_teachers_count = $conn->query("SELECT COUNT(*) as count FROM teachers");
$total_teachers_count = $total_teachers_count ? $total_teachers_count->fetch_assoc()['count'] : 0;

$active_teachers_result = $conn->query("SELECT COUNT(*) as count FROM teachers WHERE profile_completed = 1 AND status = 'active'");
$active_teachers = $active_teachers_result ? $active_teachers_result->fetch_assoc()['count'] : 0;

$pending_teachers = $conn->query("SELECT COUNT(*) as count FROM teachers WHERE (password IS NULL OR password = '') AND status = 'pending'");
...
$hindi_students = $conn->query("SELECT COUNT(*) as count FROM student_hindi");
$total_students = ($english_students ? $english_students->fetch_assoc()['count'] : 0) + 
                  ($hindi_students ? $hindi_students->fetch_assoc()['count'] : 0);

// Exam stats
$exams_result = $conn->query("SELECT COUNT(*) as count FROM exams");
$total_exams = $exams_result ? $exams_result->fetch_assoc()['count'] : 0;

// Recent activity (last 24 hours)
$recent_teachers_query = $conn->query("SELECT id, first_name, last_name, email, created_at FROM teachers WHERE created_at >= DATE_SUB(NOW(), INTERVAL 1 DAY) ORDER BY created_at DESC LIMIT 5");
...
<style>
    ...
    * {
    ...
    .progress-fill {
        ...
        border-radius: 3px;
        width: <?php echo $total_teachers_count > 0 ? ($active_teachers/$total_teachers_count)*100 : 0; ?>%;
    ...
    <main class="main-content" id="mainContent">
        ...
        <header class="header">
            ...
            <div class="header-right">
                <div class="search-bar">
                    ...
                    <input type="text" id="searchInput" name="search" placeholder="Search teachers, students..." autocomplete="off">
                </div>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo $total_teachers_count; ?></div>
        ...
        <!-- Grid for Form and Recent Activity -->
            ...
            <section class="recent-activity">
                <div class="section-header">
                    ...
                    <div>
                        ...
                        <p class="section-subtitle">Latest teacher registrations</p>
                    </div>
                </div>
                
                <ul class="activity-list">
                    <?php if ($recent_teachers_query && $recent_teachers_query->num_rows > 0): ?>
                        <?php while ($teacher = $recent_teachers_query->fetch_assoc()): ?>
                                ...
                                <div class="activity-content">
                                    <div class="activity-title">
                                        <?php 
                                            ...
                                            echo $name ? htmlspecialchars($name) : 'New Teacher';
                                        ?>
                                    </div>
                                    <div class="activity-email"><?php echo htmlspecialchars($teacher['email']); ?></div>
                                    <div class="activity-time">
                                        <i class="far fa-clock"></i>
                                        <?php echo safeDateFormat($teacher['created_at']); ?>
                                    </div>
                                </div>
                            </li>
                        <?php endwhile; ?>
        ...
        <section class="table-section">
                    ...
                    <tbody>
                        <?php if (!empty($teachers)): ?>
                            <?php foreach ($teachers as $row): ?>
                                <tr>
                                    <td data-label="ID"><strong>#<?php echo $row['id']; ?></strong></td>
                                    ...
                                    <td data-label="Email Address">
                                        ...
                                        <span style="font-weight: 600;"><?php echo htmlspecialchars($row['email']); ?></span>
                                    </td>
                                    <td data-label="Account Status">
                                        <?php if (!empty($row['password']) && $row['profile_completed']): ?>
                                            <span class="status-badge status-active">
                                                <i class="fas fa-check-circle"></i> Active
                                            </span>
                                        <?php elseif (!empty($row['password'])): ?>
                                            <span class="status-badge status-active">
                                                <i class="fas fa-check-circle"></i> Active
                                            </span>
                                        <?php else: ?>
                                    ...
                                    <td data-label="Joined Date">
                                        <span class="date-badge">
                                            ...
                                            <?php echo safeDateFormat($row['created_at']); ?>
                                        </span>
                                    </td>
                                    <?php if ($can_delete_teachers): ?>
                                    <td data-label="Actions">
                                        <form method="POST" onsubmit="return confirm('Are you sure you want to delete this teacher? This action cannot be undone.');">
                                            <input type="hidden" name="csrf_token" value="<?php echo $csrf_token; ?>">
                                            <input type="hidden" name="teacher_id" value="<?php echo $row['id']; ?>">
                                            <button type="submit" name="delete_teacher" class="btn" style="background: #e74c3c; color: white; padding: 8px 16px; font-size: 0.85rem;">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="<?php echo $can_delete_teachers ? '7' : '6'; ?>">
            ...
            <div style="margin-top: 20px; display: flex; justify-content: center; gap: 10px; flex-wrap: wrap;">
                <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                    <a href="?page=<?php echo $i; ?>" style="padding: 8px 15px; background: <?php echo $i == $page ? 'var(--primary)' : 'white'; ?>; color: <?php echo $i == $page ? 'white' : 'var(--dark)'; ?>; border-radius: 8px; text-decoration: none; border: 2px solid var(--primary-light); transition: var(--transition);">
                        <?php echo $i; ?>
                    </a>
                <?php endfor; ?>
            </div>
